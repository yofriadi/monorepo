...
export function BraceletTable({ data }: { data: LookupPrice[] }) {
  ...
  const handleEdit = async (id: string, payload: { parameter: string; value: number; }) => {
    try {
      ...
      console.error('Failed to update bracelet price:', error)
    }
  }
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      console.error('Failed to delete bracelet price:', error)
    }
  }
